...
class MeasuredAcceptanceTests(unittest.TestCase):
    ...
    def test_all_repeated_real_cases_and_cleanup_are_mandatory(self):
        ...
        rows += ['--- PASS: TestMeasuredNetworkSession'+case+' (1s)' for case in ('Normal', 'RouterLoss', 'StartupRefusal', 'DNSLoss', 'DNSRefreshFailure', 'PreparationTimeout', 'ExecutionTimeout', 'ControllerResourceLoss') for _ in range(3)]
