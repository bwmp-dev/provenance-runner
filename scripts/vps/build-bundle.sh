...
trap - EXIT
cp scripts/vps/install.sh scripts/vps/install.py "$out/"
...
(cd "$out" && sha256sum runner runsc rootfs.tar install.sh install.py prepare-gvisor-rootfs.sh settings.example.json SOURCE_COMMIT > SHA256SUMS)
