...
class JobCgroupAcceptanceTests(unittest.TestCase):
    def test_three_real_cases_required(self):
        ...
        denial = '--- PASS: TestJobCgroupKernelLifecycle/pre-cleanup-pid-denial (1s)\n'
        valid = row*3+recovery*3+denial*3
        ...
        with self.assertRaises(AssertionError):
            driver.validate_report(row*3+recovery*3, '', 0)
        with self.assertRaises(AssertionError):
            driver.validate_report(row*3+recovery*3+denial*4, '', 0)
