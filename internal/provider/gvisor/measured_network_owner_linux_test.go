...
func TestMeasuredNetworkOwnerRefusesMissingInputs(t *testing.T) {
	...
	if absent.Child() != nil || absent.Close(context.Background()) != nil || absent.Release(context.Background(), nil, nil) == nil || absent.Wait(context.Background()) == nil {
	...
	for i := 0; i < 16; i++ {
		...
		go func() {
			defer group.Done()
			for n := 0; n < 10; n++ {
				if owner.Release(context.Background(), nil, nil) == nil {
