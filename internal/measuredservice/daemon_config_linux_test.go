...
func TestDaemonConfigRequiresClosedBoundedProvisioning(t *testing.T) {
	...
	for name, mutate := range map[string]func(*DaemonConfig){
		...
		"relative-secret-root": func(c *DaemonConfig) { c.SecretRoot = "relative" },
		"broad-secret-root":    func(c *DaemonConfig) { c.SecretRoot = "/" },
		"aliased-secret-root":  func(c *DaemonConfig) { c.SecretRoot = c.BundleRoot },
